...
from kursyamls import settings
...
def export_students_performance(request, practical_work_id):
    ...
    # Фильтруем только последние попытки студентов
    latest_works = {}
    for work in completed_works:
        user_id = work['userId']
        if user_id not in latest_works or work['attemptCount'] > latest_works[user_id]['attemptCount']:
            latest_works[user_id] = work
    ...
    for work in latest_works.values():
        user_id = work['userId']
    ...
    # Имя преподавателя на другой стороне
...
def create_lection(request, subject_id):
    if request.method == 'POST':
        ...
        # Получаем метод загрузки
        upload_method = request.POST.get('upload_method', 'url')

        # Проверка наличия файла в зависимости от выбранного метода
        if upload_method == 'url':
            ...
            if not name_file_lection or not url_file_lection:
                return render(request, 'SubjectInfo/LectionCRUD.html', {
                    'error': 'Необходимо указать название файла и URL',
            ...
            unique_file_name = f"{int(time.time())}_{file_name}"
            
            # Путь для сохранения файла
            file_path = os.path.join(settings.MEDIA_ROOT, 'lection_materials', unique_file_name)
            ...
            url_file_lection = f"{settings.MEDIA_URL}lection_materials/{unique_file_name}"
            name_file_lection = file_name
    ...
    # При GET-запросе или при ошибке нужно передавать subject_id
    return render(request, 'SubjectInfo/LectionCRUD.html', {
        'subject_id': subject_id  # Добавляем subject_id в контекст
    })
...
def delete_lection(request, id):
    ...
    if response_from_api_lection.status_code != 200:
        messages.error(request, 'Лекция не найдена')
        return redirect('AllSubjectPage')
        
    lection = response_from_api_lection.json()
    subject_id = lection.get('subjectId')
    lection_name = lection.get('nameLectionMaterial', 'Лекция')

    if request.method == 'POST':
        # Удаляем лекцию без двойной проверки
        response = requests.delete(lection_url, verify=False)
        if response.status_code == 204: 
            messages.success(request, f'Лекция "{lection_name}" успешно удалена')
            return redirect('SubjectPage', id=subject_id)
        else:
            messages.error(request, 'Ошибка при удалении лекции')
            return redirect('SubjectPage', id=subject_id)
    
    # При GET-запросе просто перенаправляем на страницу предмета 
    # (все подтверждения будут выполняться через JavaScript)
    return redirect('SubjectPage', id=subject_id)
...
def login(request):
    if request.method == 'POST':
        ...
        if not login or not password:
            return render(request, 'login.html', {'error': 'Логин и пароль обязательны'})
        ...
        try:
            ...
            if users_response.status_code == 200:
                ...
                if user and bcrypt.checkpw(password.encode(), user['password'].encode()):
                    ...
                    # Check if the user is an admin (idUser == 1)
                    if user.get('idUser') == 1:
                        return redirect('adminPage')  # Redirect to AdminPanel.html
                    else:
                        return redirect('AllSubjectPage')  # Redirect to the subjects page
                else:
                    return render(request, 'login.html', {'error': 'Неверный логин или пароль'})
            else:
                return render(request, 'login.html', {'error': 'Ошибка при проверке учетных данных'})
                
        except Exception as e:
            print(f"Exception during authentication: {str(e)}")
            return render(request, 'login.html', {'error': f'Ошибка при аутентификации: {str(e)}'})
    ...
    if request.method == 'POST':
        ...
        if not email_regex.match(email):
            return render(request, 'registration.html', {
                'error': 'Неверный формат email. Пожалуйста, используйте формат example@domain.com',
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                request.session['role_id'] = id_role
                request.session['full_name'] = f"{first_name} {second_name} {middle_name}"
                
                # Check if the newly registered user is admin (idUser == 1)
                if user_data.get('idUser') == 1:
                    return redirect('adminPage')  # Redirect to AdminPanel.html
                else:
                    return redirect('nullSubjects')  # Redirect to the subjects page
            else:
...
def create_group(request):
    if request.method == 'POST':
        ...
        if not name_group or name_group.strip() == '':
            messages.error(request, 'Название группы не может быть пустым')
            return redirect('group_crud')
        
        # Проверка наличия группы с таким именем
        groups_response = requests.get(f'{API_BASE_URL}/Groups', verify=False)
        if groups_response.status_code == 200:
            existing_groups = groups_response.json()
            
            # Проверка наличия группы с таким же именем (без учета регистра)
            duplicate_groups = [group for group in existing_groups if group['nameGroup'].lower() == name_group.lower()]
            if duplicate_groups:
                duplicate_group = duplicate_groups[0]
                messages.error(request, f'Группа с названием "{duplicate_group["nameGroup"]}" уже существует (ID: {duplicate_group["idGroup"]})')
                return redirect('group_crud')
        
        # Если группы с таким именем нет, создаем новую
        response = requests.post(f'{API_BASE_URL}/Groups', json={'nameGroup': name_group}, verify=False)
        
        if response.status_code in [200, 201]:
            messages.success(request, f'Группа "{name_group}" успешно создана')
        else:
            error_message = f'Ошибка при создании группы: {response.status_code}'
            if response.text:
...
def edit_group(request, id):
    if request.method == 'POST':
        ...
        current_group_response = requests.get(f'{API_BASE_URL}/Groups/{id}', verify=False)
        current_group_name = ""
        
        if current_group_response.status_code == 200:
            current_group = current_group_response.json()
            current_group_name = current_group.get('nameGroup', '')
            
            # Если имя не изменилось, просто перенаправляем
            if current_group_name.lower() == name_group.lower():
                messages.info(request, f'Название группы "{name_group}" не было изменено')
                return redirect('group_crud')
        
        # Проверка наличия другой группы с таким именем
        groups_response = requests.get(f'{API_BASE_URL}/Groups', verify=False)
        if groups_response.status_code == 200:
            existing_groups = groups_response.json()
            
            # Проверка наличия другой группы с таким же именем (без учета регистра)
            duplicate_groups = [group for group in existing_groups 
                               if group['idGroup'] != id and group['nameGroup'].lower() == name_group.lower()]
            
            if duplicate_groups:
                duplicate_group = duplicate_groups[0]
                messages.error(request, f'Невозможно изменить название. Группа с названием "{duplicate_group["nameGroup"]}" уже существует (ID: {duplicate_group["idGroup"]})')
                return redirect('group_crud')
        
        # Если другой группы с таким именем нет, обновляем текущую
        response = requests.put(f'{API_BASE_URL}/Groups/{id}', json={'idGroup': id, 'nameGroup': name_group}, verify=False)
        
        if response.status_code == 204:
            messages.success(request, f'Группа успешно переименована с "{current_group_name}" на "{name_group}"')
        else:
            error_message = f'Ошибка при обновлении группы: {response.status_code}'
            if response.text:
                error_message += f' - {response.text[:100]}'
            messages.error(request, error_message)
...
def delete_group(request, id):
    if request.method == 'POST':
        try:
            ...
            group_response = requests.get(f'{API_BASE_URL}/Groups/{id}', verify=False)
            group_name = "группу"
            
            if group_response.status_code == 200:
                group = group_response.json()
                group_name = group.get('nameGroup', 'группу')
                
            # Проверяем, есть ли связанные предметы
            subjects_response = requests.get(f'{API_BASE_URL}/Subjects', verify=False)
            if subjects_response.status_code == 200:
                subjects = subjects_response.json()
                group_subjects = [subject for subject in subjects if subject.get('idGroup') == id]
                
                if group_subjects:
                    subject_names = [subject['nameSubject'] for subject in group_subjects]
                    subject_list = ", ".join(f'"{name}"' for name in subject_names[:3])
                    if len(subject_names) > 3:
                        subject_list += f" и ещё {len(subject_names) - 3}"
                        
                    messages.error(request, f'Невозможно удалить группу "{group_name}", так как с ней связаны предметы: {subject_list}. Сначала удалите или измените связанные предметы.')
                    return redirect('group_crud')
            ...
            if users_response.status_code == 200:
                users = users_response.json()
                group_users = [user for user in users if str(user.get('idGroup')) == str(id)]
                
                if group_users:
                    user_count = len(group_users)
                    messages.error(request, f'Невозможно удалить группу "{group_name}", так как с ней связаны {user_count} пользователей. Сначала переместите пользователей в другую группу.')
                    return redirect('group_crud')
            ...
            if response.status_code in [200, 204]:
                ...
                # Если ошибка связана с внешним ключом
                if "FK__" in response.text or "REFERENCE" in response.text:
                    messages.error(request, f'Невозможно удалить группу "{group_name}", так как с ней связаны другие записи в системе.')
                else:
                    error_message = f'Ошибка при удалении группы: {response.status_code}'
                    if response.text:
                        error_message += f' - {response.text[:100]}'
                    messages.error(request, error_message)
        except Exception as e:
...
def edit_user(request, id):
    if request.method == 'POST':
        ...
        current_user_response = requests.get(f'{API_BASE_URL}/Users/{id}', verify=False)
        if current_user_response.status_code != 200:
            messages.error(request, 'Не удалось получить данные пользователя')
        ...
        if new_email and new_email.lower() != current_user.get('email', '').lower():
            # Получаем всех пользователей для проверки email
            users_response = requests.get(f'{API_BASE_URL}/Users', verify=False)
            if users_response.status_code == 200:
                all_users = users_response.json()
                
                # Проверяем, существует ли пользователь с таким email
                duplicate_users = [user for user in all_users if user['idUser'] != id and user.get('email', '').lower() == new_email.lower()]
                
                if duplicate_users:
                    duplicate_user = duplicate_users[0]
                    duplicate_name = f"{duplicate_user.get('firstName', '')} {duplicate_user.get('secondName', '')}"
                    messages.error(request, f'Пользователь с такой электронной почтой уже существует: {duplicate_name} (ID: {duplicate_user["idUser"]})')
                    return redirect('user_crud')
        ...
        # Не обновляем логин и пароль, так как они не изменяются в этой форме
        if 'login' in current_user:
            user_data['Login'] = current_user['login']
        if 'password' in current_user:
            user_data['Password'] = current_user['password']
        
        print("Данные для обновления пользователя:", user_data)
        
        try:
            response = requests.put(f'{API_BASE_URL}/Users/{id}', json=user_data, verify=False)
            
            if response.status_code == 204: 
                messages.success(request, f'Пользователь {user_data["FirstName"]} {user_data["SecondName"]} успешно обновлен')
                return redirect('user_crud')
            else:
                # Проверяем, содержит ли ответ информацию о дублировании email
                error_text = response.text.lower() if response.text else ""
                if "email" in error_text and ("exists" in error_text or "duplicate" in error_text or "already" in error_text):
                    messages.error(request, f'Пользователь с email {new_email} уже существует в системе')
                else:
                    messages.error(request, f'Ошибка при обновлении данных пользователя: {response.status_code}')
...
def delete_user(request, id):
    if request.method == 'POST':
        try:
            ...
            user_response = requests.get(f'{API_BASE_URL}/Users/{id}', verify=False)
            user_info = ""
            
            if user_response.status_code == 200:
                user = user_response.json()
                user_info = f"{user.get('firstName', '')} {user.get('secondName', '')}"
            
            deleted_related_records = False
            deleted_count = 0
            
            # Удаляем связанные работы пользователя (CompletedWork)
            completed_works_response = requests.get(f'{API_BASE_URL}/CompletedWorks', verify=False)
            if completed_works_response.status_code == 200:
                completed_works = completed_works_response.json()
                user_works = [work for work in completed_works if str(work.get('userId')) == str(id)]
                
                for work in user_works:
                    work_id = work.get('idCompletedWork')
                    delete_response = requests.delete(f'{API_BASE_URL}/CompletedWorks/{work_id}', verify=False)
                    if delete_response.status_code in [200, 204]:
                        deleted_count += 1
                        deleted_related_records = True
                        print(f"Удалена работа с ID: {work_id}")
                    else:
                        print(f"Ошибка при удалении работы с ID {work_id}: {delete_response.status_code}")
            
            # Удаляем связанные попытки прохождения тестов (TestAttempts)
            test_attempts_response = requests.get(f'{API_BASE_URL}/TestAttempts', verify=False)
            if test_attempts_response.status_code == 200:
                test_attempts = test_attempts_response.json()
                user_attempts = [attempt for attempt in test_attempts if str(attempt.get('userId')) == str(id)]
                
                for attempt in user_attempts:
                    attempt_id = attempt.get('idTestAttempt')
                    delete_response = requests.delete(f'{API_BASE_URL}/TestAttempts/{attempt_id}', verify=False)
                    if delete_response.status_code in [200, 204]:
                        deleted_count += 1
                        deleted_related_records = True
                        print(f"Удалена попытка прохождения теста с ID: {attempt_id}")
                    else:
                        print(f"Ошибка при удалении попытки с ID {attempt_id}: {delete_response.status_code}")
            
            # Отправляем запрос на удаление самого пользователя
            response = requests.delete(f'{API_BASE_URL}/Users/{id}', verify=False)
            
            if response.status_code in [200, 204]:
                # Успешное удаление
                success_msg = f'Пользователь {user_info} успешно удален'
                if deleted_related_records:
                    success_msg += f' вместе со связанными записями ({deleted_count} шт.)'
                messages.success(request, success_msg)
            else:
                # Если всё еще возникает ошибка с внешними ключами
                if "FK__" in response.text or "REFERENCE" in response.text:
                    # Анализируем текст ошибки для определения проблемной таблицы
                    error_text = response.text.lower()
                    problem_table = "других таблиц"
                    
                    if "subject" in error_text:
                        problem_table = "Subjects (предметы)"
                    elif "practical" in error_text:
                        problem_table = "PracticalWorks (практические работы)"
                    elif "lection" in error_text:
                        problem_table = "LectionMaterials (лекции)"
                    elif "test" in error_text:
                        problem_table = "Tests (тесты)"
                    
                    messages.error(request, f'Невозможно удалить пользователя {user_info}, так как с ним связаны записи в таблице {problem_table}. Сначала необходимо изменить или удалить эти записи.')
                else:
                    # Общая ошибка при удалении
                    error_msg = f'Ошибка при удалении пользователя: {response.status_code}'
                    if response.text:
                        error_msg += f" - {response.text[:200]}"  # Ограничиваем длину текста ошибки
                    
                    messages.error(request, error_msg)
                
        except requests.exceptions.RequestException as e:
            error_msg = f'Ошибка соединения с сервером при удалении пользователя: {str(e)}'
            messages.error(request, error_msg)
...
def create_practical_work(request, subject_id):
    if request.method == 'POST':
        ...
        # Получаем метод загрузки напрямую из радио-кнопки
        upload_method = request.POST.get('upload_method', 'url')
        print(f"Выбранный метод загрузки: {upload_method}")

        # Переменные для файла - инициализируем заранее
        name_file_practical_work = None
        url_file_practical_work = None
        ...
        if upload_method == 'url':
            ...
            print(f"URL метод: имя файла = '{name_file_practical_work}', url = '{url_file_practical_work}'")
            
            # Проверяем, что оба поля заполнены
            if not name_file_practical_work or not url_file_practical_work:
            ...
            print(f"Файловый метод: проверяем наличие загруженного файла")
            ...
            # Путь для сохранения файла
            file_path = os.path.join(settings.MEDIA_ROOT, 'practical_works', unique_file_name)
            ...
            # URL для доступа к файлу
            url_file_practical_work = f"{settings.MEDIA_URL}practical_works/{unique_file_name}"
            name_file_practical_work = file_name
        ...
        print(f"Отправляемые данные в API: {data}")

        try:
            response = requests.post(api_url, json=data, verify=False)
            print(f"Ответ API: {response.status_code}, {response.text[:200]}")
            response.raise_for_status()
            return redirect('SubjectPage', id=subject_id)
        except requests.exceptions.RequestException as e:
            print(f"Ошибка при отправке запроса: {e}")
            return render(request, 'SubjectInfo/CRUDPracticalWork.html', {
                'error': f'Ошибка при добавлении практической работы: {str(e)}',
                'subject_id': subject_id,
                'form_data': {
                    'namePracticalWork': name_practical_work,
                    'descriptionPracticalWork': description_practical_work
    ...
    return render(request, 'SubjectInfo/CRUDPracticalWork.html', {'subject_id': subject_id})
        ...
        if completed_works_response['status_code'] == 200:
            ...
            if students_response['status_code'] == 200:
                ...
                if role_id == 3:  # Если студент
                    ...
                    # Сортируем список работ по attemptCount в порядке убывания (от новых к старым)
                    # Это обеспечит, что в шаблоне forloop.first будет указывать на самую новую попытку
                    completed_works.sort(key=lambda x: int(x.get('attemptCount', 0)), reverse=True)
...
def delete_practical_work(request, id):
    try:
        ...
        response = requests.get(practical_work_url, verify=False)
        
        if response.status_code != 200:
            messages.error(request, 'Практическая работа не найдена')
            return redirect('AllSubjectPage')
            
        practical_work = response.json()
        subject_id = practical_work.get('subjectId')
        name_practical_work = practical_work.get('namePracticalWork', 'Практическая работа')
        
        # Удаляем все связанные работы студентов
        deleted_count = 0
        
        # 1. Получаем все выполненные работы студентов
        completed_works_url = f'{API_BASE_URL}/CompletedWorks'
        completed_works_response = requests.get(completed_works_url, verify=False)
        
        if completed_works_response.status_code == 200:
            all_completed_works = completed_works_response.json()
            # Фильтруем только те работы, которые связаны с удаляемой практической работой
            related_works = [
                work for work in all_completed_works
                if str(work.get('practicalWorkId')) == str(id)
            ]
            
            # 2. Удаляем каждую связанную работу без вывода сообщений
            for work in related_works:
                work_id = work.get('idCompletedWork')
                delete_work_url = f'{API_BASE_URL}/CompletedWorks/{work_id}'
                delete_work_response = requests.delete(delete_work_url, verify=False)
                
                if delete_work_response.status_code == 204:
                    deleted_count += 1
                else:
                    print(f"Ошибка при удалении работы {work_id}: {delete_work_response.status_code}")
            
            # 3. Теперь удаляем саму практическую работу
            delete_response = requests.delete(practical_work_url, verify=False)
            
            if delete_response.status_code == 204:
                # Выводим только одно сообщение, независимо от количества удаленных работ
                if deleted_count > 0:
                    messages.success(request, f'Практическая работа "{name_practical_work}" и связанные с ней работы студентов успешно удалены')
                else:
                    messages.success(request, f'Практическая работа "{name_practical_work}" успешно удалена')
                return redirect('SubjectPage', id=subject_id)
            else:
                messages.error(request, f'Ошибка при удалении практической работы: {delete_response.status_code}')
        else:
            # Если не удалось получить список работ, пробуем удалить только саму практическую работу
            delete_response = requests.delete(practical_work_url, verify=False)
        ...
        print(f"Ошибка при удалении практической работы: {str(e)}")
        messages.error(request, 'Произошла ошибка при удалении практической работы')
        return redirect('AllSubjectPage')
...
def AddCompleteWork(request, practical_work_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if upload_method == 'url':
                # Стандартная отправка через ссылку
                ...
                unique_id = f"{int(time.time())}_{user_id}"
                unique_filename = f"{unique_id}{file_extension}"
                
                # Создаем папку для сохранения файлов студентов, если её нет
                upload_dir = os.path.join(settings.MEDIA_ROOT, 'completed_works')
                ...
                file_url = f"{settings.MEDIA_URL}completed_works/{unique_filename}"
                
                # Отправляем работу с корректным attempt_count и ссылкой на загруженный файл
                result = send_completed_work_direct(user_id, practical_work_id, original_filename, file_url)
...
def grade_completed_work(request, practical_work_id, user_id):
    try:
        ...
        # Проверяем, имеет ли пользователь права преподавателя
        if request.session.get('role_id') != 2:  # 2 - роль преподавателя
            return HttpResponse("У вас нет прав для оценивания работ", status=403)
        
        # Получение данных о пользователе для отображения имени
        user_response = make_api_request(f"Users/{user_id}")
        student_name = ""
        if user_response['status_code'] == 200 and user_response['data']:
            student = user_response['data']
            student_name = f"{student.get('lastName', '')} {student.get('firstName', '')} {student.get('middleName', '')}"
        ...
        # Добавляем имя студента в объект работы для отображения в шаблоне
        completed_work['student_name'] = student_name
        ...
        if request.method == 'POST':
            ...
            feedback = request.POST.get('feedback', '')  # Получаем комментарий, если он есть
            print(f"Полученная оценка: {grade}, комментарий: {feedback}")
            ...
            # Добавляем комментарий к работе, если он был предоставлен
            if feedback:
                updated_work['teacherComment'] = feedback
...
def edit_test(request, id):
    if request.method == 'POST':
        try:
            ...
            print("Начинаем обработку вопросов теста")
            questions_url = f'{API_BASE_URL}/QuestionsTests'

            # Обработка удаленных вопросов
            deleted_questions = request.POST.getlist('deleted_questions[]')
            for q_id in deleted_questions:
                if q_id and q_id.isdigit():
                    delete_url = f'{questions_url}/{q_id}'
                    try:
                        response = requests.delete(delete_url, verify=False)
                        print(f"Удаление вопроса {q_id}: статус {response.status_code}")
                    except Exception as e:
                        print(f"Ошибка при удалении вопроса {q_id}: {str(e)}")
            ...
            if questions_response['status_code'] == 200:
                for q in questions_response['data']:
                    if q.get('idTest') == int(id):
                        existing_questions[str(q.get('idQuestion'))] = q

            # Обрабатываем данные формы для вопросов
            for key, value in request.POST.items():
                if key.startswith('questions[') and '][text]' in key:
                    # Извлекаем ID вопроса из имени поля
                    question_id = key.split('[')[1].split(']')[0]
                    
                    # Получаем данные вопроса
                    question_text = request.POST.get(f'questions[{question_id}][text]')
                    question_type = int(request.POST.get(f'questions[{question_id}][type]'))
                    question_points = int(request.POST.get(f'questions[{question_id}][points]'))
                    
                    # Подготавливаем correct_answer и answer_variants в зависимости от типа вопроса
                    correct_answer = ''
                    answer_variants = ''
                    
                    if question_type == 1 or question_type == 2:  # Один или несколько правильных ответов
                        answers = request.POST.getlist(f'questions[{question_id}][answers][]')
                        answer_variants = '; '.join(answers) if answers else ''
                        
                        # Для одиночного выбора
                        if question_type == 1:
                            selected_index = request.POST.get(f'questions[{question_id}][correct]')
                            if selected_index is not None and selected_index.isdigit():
                                selected_index = int(selected_index)
                                if 0 <= selected_index < len(answers):
                                    correct_answer = answers[selected_index]
                        # Для множественного выбора
                        else:
                            correct_answer = request.POST.get(f'questions[{question_id}][correctAnswer]', '')
                            
                    elif question_type == 3:  # Сопоставление
                        left_parts = request.POST.getlist(f'questions[{question_id}][left][]')
                        right_parts = request.POST.getlist(f'questions[{question_id}][right][]')
                        
                        if len(left_parts) == len(right_parts):
                            pairs = []
                            for i in range(len(left_parts)):
                                pairs.append(f"{left_parts[i]} - {right_parts[i]}")
                            answer_variants = '; '.join(pairs)
                            correct_answer = answer_variants  # Для сопоставления правильный ответ совпадает с вариантами
                            
                    elif question_type == 4:  # Ввод ответа
                        correct_answer = request.POST.get(f'questions[{question_id}][correct_answer]', '')
                        answer_variants = ''  # Для ввода ответа нет вариантов
                        
                    elif question_type == 5:  # Установление порядка
                        order_items = request.POST.getlist(f'questions[{question_id}][order][]')
                        answer_variants = '; '.join(order_items)
                        correct_answer = answer_variants  # Для порядка правильный ответ - это исходный порядок
                    ...
                    # Если это существующий вопрос (не новый), обновляем его
                    if question_id in existing_questions and not question_id.startswith('new_'):
                        question_data['idQuestion'] = int(question_id)
                        update_url = f'{questions_url}/{question_id}'
                        try:
                            response = requests.put(update_url, json=question_data, verify=False)
                            print(f"Обновление вопроса {question_id}: статус {response.status_code}")
                            if response.status_code not in [200, 204]:
                                print(f"Ошибка при обновлении вопроса: {response.text}")
                        except Exception as e:
                            print(f"Ошибка при обновлении вопроса {question_id}: {str(e)}")
                    else:
                        # Если это новый вопрос, создаем его
                        try:
                            # Удаляем ненужное поле idQuestion для новых вопросов
                            if 'idQuestion' in question_data:
                                del question_data['idQuestion']
                            
                            response = requests.post(questions_url, json=question_data, verify=False)
                            print(f"Создание нового вопроса: статус {response.status_code}")
                            if response.status_code not in [200, 201]:
                                print(f"Ошибка при создании вопроса: {response.text}")
                        except Exception as e:
...
def check_single_choice(request, question_id, question):
    ...
    # Если правильный ответ не определен и пользователь не дал ответа - засчитываем как верный
    if (not correct_answer or correct_answer.strip() == '') and (not user_answer or user_answer.strip() == ''):
        print("Правильный ответ не определен и пользователь не ответил - засчитываем как верный")
        return question.get('points', 0)
...
def check_multiple_choice(request, question_id, question):
    ...
    # Если правильные ответы не определены и пользователь не дал ответа - засчитываем как верный
    if (not correct_answers or len(correct_answers) == 0) and (not user_answers or len(user_answers) == 0):
        print("Правильные ответы не определены и пользователь не ответил - засчитываем как верный")
        return question.get('points', 0)
...
def check_text_input(request, question_id, question):
    ...
    # Если правильный ответ не определен и пользователь не дал ответа - засчитываем как верный
    if (not correct_answer) and (not user_answer):
        print("Правильный ответ не определен и пользователь не ответил - засчитываем как верный")
        return question.get('points', 0)
...
def check_matching(request, question_id, question):
    ...
    # Если правильные пары не определены и пользователь не дал ответа - засчитываем как верный
    if (not correct_pairs or len(correct_pairs) == 0) and (not user_pairs or len(user_pairs) == 0):
        print("Правильные пары не определены и пользователь не ответил - засчитываем как верный")
        return question.get('points', 0)
...
def check_ordering(request, question_id, question):
    ...
    # Если правильный порядок не определен и пользователь не дал ответа - засчитываем как верный
    if (not correct_order or len(correct_order) == 0) and (not user_order or len(user_order) == 0):
        print("Правильный порядок не определен и пользователь не ответил - засчитываем как верный")
        return question.get('points', 0)
...
def reset_test_attempt(request, test_id, user_id):
    """
    Функция для аннулирования последней попытки прохождения теста студентом.
    ...
    # Проверяем, имеет ли пользователь права на аннулирование (преподаватель или админ)
    role_id = request.session.get('role_id')
    if role_id not in [1, 2]:  # 1 - админ, 2 - преподаватель
    ...
    if request.method == 'POST':
        try:
            ...
            if response_attempts.status_code != 200:
                messages.error(request, 'Не удалось получить информацию о попытках')
                return redirect('test_students_results', id=test_id)
            ...
            user_attempts = [
                attempt for attempt in all_attempts 
                if str(attempt.get('testId')) == str(test_id) and str(attempt.get('userId')) == str(user_id)
            ]
            ...
            # Проверяем, что у пользователя есть попытки
            if not user_attempts:
                messages.error(request, 'У студента нет попыток прохождения этого теста')
                return redirect('test_students_results', id=test_id)
            
            # Получаем последнюю попытку
            last_attempt = user_attempts[0]
            attempt_id = last_attempt.get('idTestAttempt')
            
            # Проверяем статус последней попытки (она должна быть неудачной)
            grade = float(last_attempt.get('grade', 0))
            if grade >= 3:
                messages.error(request, 'Последняя попытка студента успешная. Нельзя аннулировать успешную попытку.')
                return redirect('test_students_results', id=test_id)
            
            # Удаляем последнюю попытку
            delete_url = f'{API_BASE_URL}/TestAttempts/{attempt_id}'
            response_delete = requests.delete(delete_url, verify=False)
            
            if response_delete.status_code == 204:
                messages.success(request, 'Последняя попытка успешно аннулирована. Студент может пройти тест снова.')
            else:
                messages.error(request, f'Ошибка при аннулировании попытки: {response_delete.status_code}')
            
            return redirect('test_students_results', id=test_id)
        
        except Exception as e:
            print(f"Ошибка при аннулировании попытки: {e}")
            messages.error(request, f'Произошла ошибка: {str(e)}')
            return redirect('test_students_results', id=test_id)
    ...
    user_response = make_api_request(f'Users/{user_id}')
    user = user_response['data'] if user_response['status_code'] == 200 else None
    
    # Получаем данные теста
    test_response = make_api_request(f'Tests/{test_id}')
    test = test_response['data'] if test_response['status_code'] == 200 else None
    
    # Получаем последнюю попытку
    attempts_response = make_api_request('TestAttempts')
    last_attempt = None
    
    if attempts_response['status_code'] == 200:
        user_attempts = [
            attempt for attempt in attempts_response['data'] 
            if str(attempt.get('testId')) == str(test_id) and str(attempt.get('userId')) == str(user_id)
        ]
    ...
    # Если в last_attempt нет maxScore, попытаемся его рассчитать
    if last_attempt and not last_attempt.get('maxScore') and test:
        # Получаем вопросы теста для подсчета максимального количества баллов
        questions_response = make_api_request(f'Questions?testId={test_id}')
        if questions_response['status_code'] == 200:
            questions = questions_response['data']
            # Считаем сумму баллов за все вопросы
            max_score = sum(float(question.get('points', 0)) for question in questions)
            # Добавляем maxScore в last_attempt
            last_attempt['maxScore'] = max_score
    ...
    return render(request, 'SubjectInfo/ConfirmResetAttempt.html', context)
